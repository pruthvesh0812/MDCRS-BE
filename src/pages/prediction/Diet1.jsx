...
export default function Diet1() {
  ...
  const fruitsVeggies = [
    { label: 'Both Fruits and vegetables Everday', freq: 'Never', value: 0 },
    { label: 'Only Fruits Everday', freq: 'Never', value: 1 },
    { label: 'Only vegetables Everday', freq: 'Never', value: 2 },
    { label: 'I do not eat fruits and vegetables everyday', freq: 'Never', value: 3 },

  ]

  const junk_options = [
    { label: 'Never', junk: 'Never', value: 'Never' },
    { label: 'Once in a while', junk: 'Occasionally', value: 'Occasionally' },
    { label: 'More than 4 times a month', junk: 'Frequently', value: 'Frequently' },
  ];
  ...
  return (
    <div className='mx-[30%] my-[10%]'>
      <form onSubmit={handleSubmit}>
        <div className='mt-10 '>
          <h3 className='font-Roboto text-md font-semibold mb-1'>What is your specific diet preference?</h3>
          <form>
        ...
        <div className='mt-10'>
          <h3 className='font-Roboto text-md font-semibold mb-1'>How often do you find yourself consuming items high in sugar, salt, or processed foods (candies, chips, cakes, burger, pizza)?</h3>
          <div className=''>
            <Select
              options={junk_options}
              value={junk_options.value}
              placeholder='Select junk food frequency'
              onChange={(value) => { setUserLifestyle((prev) => ({ ...prev, JunkFood: value.value })) }}
              isSearchable={true}
            />
          </div>
        </div>
        ...
        <div className='mt-10'>
          <h3 className='font-Roboto text-md font-semibold mb-1'>Do you eat Fruits and Vegetables Everyday?</h3>
          <div className=''>
            <Select
              options={fruitsVeggies}
              value={fruitsVeggies.value}
              placeholder='Select one option'
              onChange={(ele) => {
                console.log(ele.value, 'values')
                if (ele.value == 0) {
                  setUserLifestyle((prev) => ({ ...prev, Fruits: 1 }))
                  setUserLifestyle((prev) => ({ ...prev, Veggies: 1 }))
                }
                else if (ele.value == 1) {
                  setUserLifestyle((prev) => ({ ...prev, Fruits: 1 }))
                  setUserLifestyle((prev) => ({ ...prev, Veggies: 0 }))
                }
                else if (ele.value == 2) {
                  setUserLifestyle((prev) => ({ ...prev, Fruits: 0 }))
                  setUserLifestyle((prev) => ({ ...prev, Veggies: 1 }))
                }
                else {
                  setUserLifestyle((prev) => ({ ...prev, Fruits: 0 }))
